...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Product.recipe'
        db.delete_column('products', 'recipe_id')

        # Adding M2M table for field recipe on 'Product'
        m2m_table_name = db.shorten_name('products_recipe')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('product', models.ForeignKey(orm[u'products.product'], null=False)),
            ('recipes', models.ForeignKey(orm[u'recipe.recipes'], null=False))
        ))
        db.create_unique(m2m_table_name, ['product_id', 'recipes_id'])


    def backwards(self, orm):
        # Adding field 'Product.recipe'
        db.add_column('products', 'recipe',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, related_name='products', to=orm['recipe.Recipes']),
                      keep_default=False)

        # Removing M2M table for field recipe on 'Product'
        db.delete_table(db.shorten_name('products_recipe'))
